...
function tampil() {
  ...
  tabel.innerHTML = "<tr><th>Tipe</th><th>Event</th><th>Bulan</th><th>Kota</td><th>Harga</th><th>Jumlah</th><th>Total</th><th colspan='2'>Aksi</th></tr>";
  for (let i = 0; i < obj.length; i++) {
    var btnTambah = "<button class='btn-tambah' onclick='tambah('+i+')' type='button'>+</button>";
    var btnKurang = "<button class='btn-kurang' onclick='kurang('+i+')' type='button'>-</button>";
    tabel.innerHTML += `<tr><td>${obj[i].eventType}</td><td>${obj[i].eventName}</td><td>${obj[i].month}</td><td>${obj[i].city}</td><td>${obj[i].price}</td><td>Jumlah</td><td>Total</td><td>${btnTambah}</td><td>${btnKurang}</td></tr>`;
  }
  td;
}
